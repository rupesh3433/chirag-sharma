...
import {
  ...
  Link, Zap, Timer, Flame, Star, Layers,
} from "lucide-react";
...
export function VisitorsTab() {
  ...
  const { data: vd, isLoading, refetch } = useQuery({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <PeriodRow vd={vd} loading={isLoading} />
      ...
      <div className="rg g4">
        {[
          { lbl: "Avg Duration",    val: fmtDur(vd?.avg_duration_seconds ?? 0), icon: Timer,         color: P.violet, sub: "per session"       },
          { lbl: "Longest Session", val: fmtDur(vd?.max_duration_seconds ?? 0), icon: Star,          color: P.amber,  sub: "today"             },
          { lbl: "Bounced",         val: fmt(vd?.bounce_count_today ?? 0),      icon: Zap,           color: P.coral,  sub: "left after 1 page" },
          { lbl: "Power Users",     val: fmt(vd?.power_count_today ?? 0),       icon: Flame,         color: P.teal,   sub: "4+ pages · 2min+"  },
      ...
      {(vd?.hourly_today?.length ?? 0) > 0 && (
        ...
        >
          <ResponsiveContainer width="100%" height={chS}>
            <BarChart data={vd.hourly_today} margin={{ top: 8, right: 4, bottom: 0, left: -20 }}>
              ...
              <Bar dataKey="unique" name="Sessions" radius={[5, 5, 0, 0]}>
                {(vd.hourly_today ?? []).map((_: any, i: number) => (
                  <Cell key={i} fill={P.violet} fillOpacity={0.25 + (i / (vd.hourly_today.length || 1)) * 0.75} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          {(vd?.top_referrers?.length ?? 0) === 0 ? (
            ...
            vd.top_referrers.map((r: any, i: number) => (
              <BarProgress
                ...
                value={r.count} max={vd.top_referrers[0].count}
                color={CHART_COLORS[i % CHART_COLORS.length]}
              />
            ))
          )}
