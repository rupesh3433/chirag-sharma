// ================================================================
// OverviewTab.tsx — High-level summary combining visitors, bookings,
//                   counter stats, and live viewer snapshot.
// ================================================================
...
export function OverviewTab() {
      ...
      try {
        ...
        ws.onmessage = (event: MessageEvent) => {
          try {
            ...
            if (data && data.type === "live_count" && typeof data.count === "number") {
              setLiveCount(data.count);
            }
          } catch { return; }
        };
        ws.onclose = () => {
          clearPing(); wsRef.current = null;
          if (!destroyed) { clearReconnect(); reconnectTimeoutRef.current = setTimeout(connect, 3000); }
        };
        ws.onerror = () => ws.close();
      } catch {
        if (!destroyed) { clearReconnect(); reconnectTimeoutRef.current = setTimeout(connect, 5000); }
      }
    };

    connect();
    return () => {
      destroyed = true; clearPing(); clearReconnect();
  ...
  const { data: vd, isLoading: vdL } = useQuery({
        ...
        {[
          ...
          <div key={lbl} className="stat-card" style={{ "--stripe": color } as any}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
              <span style={{ fontSize: 9.5, fontWeight: 800, color, textTransform: "uppercase", letterSpacing: ".1em", flex: 1, paddingRight: 8 }}>
                {lbl}
              </span>
          ...
          <div key={lbl} className="stat-card" style={{ "--stripe": color } as any}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 9.5, fontWeight: 800, color, textTransform: "uppercase", letterSpacing: ".1em", margin: "0 0 11px" }}>
                  {lbl}
                </p>
                {!cs ? <Skel h={34} w={60} /> : (
          ...
          <div key={label} className="stat-card" style={{ "--stripe": color } as any}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p className="mono num-qual" style={{ fontWeight: 700, color: "#111827", margin: 0 }}>
                  {loading ? "—" : v}
                </p>
              </div>
              <div style={{ width: 34, height: 34, borderRadius: 10, background: `${color}14`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginLeft: 8 }}>
            ...
            <ResponsiveContainer width="100%" height={chS}>
              <BarChart data={monthlyChart} margin={{ top: 8, right: 4, bottom: 0, left: -20 }}>
                ...
                <Bar dataKey="Bookings" radius={[6, 6, 0, 0]}>
                  {monthlyChart.map((_: any, i: number) => (
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {loading ? <Skel h={34} w={55} /> : (
                  <p className="mono num-lg" style={{ fontWeight: 700, color: "#111827", lineHeight: 1, margin: "0 0 4px" }}>
                    <Num v={v} />
                  </p>
                )}
