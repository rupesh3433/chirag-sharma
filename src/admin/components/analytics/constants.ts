...
/* Root container — NO overflow set (would break fixed positioning context)
   flex: 1 1 auto makes it fill the remaining width of its flex parent
   (SidebarInset / the admin <main>), matching the fixed header exactly. */
.an {
  font-family: 'DM Sans', system-ui, sans-serif;
  color: #111827;
  background: #fff;   /* white so the header-spacer gap has no colour bleed */
  -webkit-font-smoothing: antialiased;

  /* Stretch to fill the flex parent that sits next to the sidebar */
  width: 100%;
  max-width: 100%;
  flex: 1 1 auto;
...
.an-content-clip {
  overflow-x: clip;
  background: #fff;   /* white canvas for tab content only; header keeps its own bg */
  width: 100%;
}
...
/* ── Layout wrappers ──────────────────────────────────────────────
   .an-wrap  : used inside the FIXED header bar — provides horizontal
               padding so title / badge / tab-strip are inset from the
               bar edges.  The tab-strip uses equal-but-negative margins
               to "bleed" its underline back to those edges.
               DO NOT change the horizontal padding here without also
               updating the tab-strip bleed values in Analytics.tsx.

   .an-content : used for scrolling tab content — sits INSIDE .an-wrap,
                 so .an-wrap already contributes the horizontal padding.
                 Only a top-padding offset is needed here; adding
                 horizontal padding would create double-padding and push
                 content further in than the header, causing the
                 visible width mismatch.
── */
.an-wrap {
  padding: 0 28px;        /* horizontal inset for header elements */
  width: 100%;
  box-sizing: border-box;
}
.an-content {
  padding: 24px 0 0;      /* top offset only — horizontal comes from .an-wrap */
  width: 100%;
  box-sizing: border-box;
}
...
@media (max-width: 1024px) {
  /* .an-wrap changes horizontal padding → tab-strip bleed in Analytics.tsx
     already accounts for this via its own media query. */
  .an-wrap    { padding-left: 20px; padding-right: 20px; }
  /* .an-content: horizontal padding is inherited from .an-wrap; only
     update top spacing here. */
...
@media (max-width: 640px) {
  .an-wrap    { padding-left: 14px; padding-right: 14px; }
...
@media (max-width: 400px) {
  .an-wrap    { padding-left: 10px; padding-right: 10px; }
