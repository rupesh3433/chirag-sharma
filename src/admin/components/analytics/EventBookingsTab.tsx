...
export function EventBookingsTab() {
  ...
  const {
    ...
    ["analytics-evt", efrom, eto] as const,
    () =>
  ...
  const totalEvt   = evtAny?.total ?? 0;
  const paidEvt    = (evtAny?.by_status?.paid ?? 0) + (evtAny?.by_status?.confirmed ?? 0);
  const revDisplay = Math.round((evtAny?.total_revenue_units ?? 0) / 100);
  const convRate   = pct(paidEvt, totalEvt);

  const evtStatusPie = Object.entries(evtAny?.by_status ?? {}).map(([k, v]) => ({
    name:  k.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
    value: v as number,
  }));

  const providerD = (evtAny?.by_provider ?? []).map((p: any) => ({
  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <TabShell loading={loading} timedOut={timedOut} accentColor={P.violet}>
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          ...
          <div className="rg g4">
            {[
              ...
              { label: "Checked In",       v: evtAny?.by_status?.["checked_in"] ?? 0,  icon: UserCheck,  color: P.sky    },
              { label: "Cancelled",        v: evtAny?.by_status?.cancelled ?? 0,       icon: XCircle,    color: P.coral  },
          ...
          <div className="rg g3">
            {[
              ...
              { label: "Today's Event Bookings", val: fmt(evtAny?.today_event_bookings ?? 0), color: P.fuchsia, sub: "New event bookings today"         },
          ...
          {/* Top events */}
          {(evtAny?.by_event?.length ?? 0) > 0 && (
            <ChartCard
              title="Bookings by Event" icon={Star}
              accentColor={P.fuchsia}
              subtitle="Top events ranked by booking count"
            >
              {evtAny.by_event.slice(0, 8).map((e: any, i: number) => (
                <BarProgress
                  key={i}
                  label={e.event}
                  value={e.count}
                  max={evtAny.by_event[0].count}
            ...
            <div className="rg g-charts">
              ...
              >
                <ResponsiveContainer width="100%" height={ch}>
                  <AreaChart data={evtAny.daily_trend} margin={{ top: 8, right: 4, bottom: 0, left: -20 }}>
                    <defs>
              ...
              >
                <ResponsiveContainer width="100%" height={ch}>
                  <AreaChart
                    data={(evtAny.daily_trend ?? []).map((d: any) => ({
                      ...d,
